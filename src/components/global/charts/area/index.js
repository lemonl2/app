...
class AreaChartController {
      ...
      switch (type) {
        case 'option' : {
          ...
          if (this.data && this.data.valueKeys.length) {
            dealOption(ecChangedOption, this.data, 'area');
  ...
  fetchData() {
    ...
    .then(({ data }) => {
      ...
      if (this.data && this.data.valueKeys.length) {
        dealOption(this.ecOption, this.data, 'area');
        //触发图表刷新
...
AreaChartController.$inject = ['$rootScope', '$scope', '$state', 'SplService', 'config'];

app.component('areaChart', {
